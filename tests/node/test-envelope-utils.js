...
// window.PGEEnv at IIFE time and reads window.PGEDeviationProbability (deviation-probability.js) at call
...
console.log("\n── deviation_probability envelopes (issue #61) ──");
{
  ...
  assert("rescale deviation_probability array form", eq(ra.deviationProbability, [[0, 0], [0.5, 1]]));
  assert("streamWouldTruncate true on deviation_probability array", U.streamWouldTruncate(arrStream, 2) === true);
  ...
  assert("rescale deviation_probability.volume (array param)", eq(ro.deviationProbability.volume, [[0, 0], [0.5, 1]]));
  assert("deviation_probability.pan scalar param preserved",   ro.deviationProbability.pan === 0.5);
  assert("deviation_probability.pitch null param preserved",   ro.deviationProbability.pitch === null);
  assert("streamWouldTruncate true on deviation_probability.volume env", U.streamWouldTruncate(objStream, 2) === true);
  assert("rescaleStreamEnvelopes does not mutate deviation_probability input", eq(objStream.deviationProbability.volume, [[0, 0], [1, 1]]));

  // scalar / false deviation_probability left untouched (not a time-domain envelope)
  assert("deviation_probability scalar untouched", U.rescaleStreamEnvelopes({ deviationProbability: 0.01 }, 10, 20).deviationProbability === 0.01);
  assert("deviation_probability false untouched",  U.rescaleStreamEnvelopes({ deviationProbability: false }, 10, 20).deviationProbability === false);
}
...
console.log("\n── deviation_probability typed {type,points} envelopes (cubic global interp) ──");
{
  ...
  assert("rescale deviation_probability typed global env (points scaled)",
    eq(rt.deviationProbability, { type: "cubic", points: [[0, 0], [0.5, 1]] }));
  assert("streamWouldTruncate true on typed global deviation_probability env",
    U.streamWouldTruncate(typedStream, 2) === true);
  assert("rescale does not mutate typed deviation_probability input",
  ...
  assert("truncate typed global deviation_probability env clips past 1.0",
  ...
  assert("rescale deviation_probability.volume typed env param",
    eq(rp.deviationProbability.volume, { type: "cubic", points: [[0, 0], [0.5, 1]] }));
  assert("deviation_probability.pan scalar preserved alongside typed param", rp.deviationProbability.pan === 0.5);
  assert("streamWouldTruncate true on typed per-param deviation_probability env",
