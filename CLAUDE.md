...
- **`make tests-node`** (node, no deps beyond npm) — `tests/node/test-yaml-bridge.js`
  ...
  engine `configs/*.yml` as fixtures when present — including the healing of the
  pre-v7 `dephase` spelling into `deviation_probability`), `test-envelope-utils.js`
...
Editor in-memory shape is camelCase JS with **parallel scalar/envelope fields** (e.g. `density` is a number, `densityEnv` is an array — exactly one is non-null). YAML on disk is snake_case with a **single field** that's either scalar OR envelope. `parse()` and `serialize()` translate between the two. Unknown stream keys are preserved verbatim under `_extra` so the round trip stays lossless for fields the editor doesn't model; unknown keys *inside* `pointer`/`grain`/`pitch`/`voices` (blocks the serializer rebuilds in full) are preserved the same way under `<block>._extra`. `deviation_probability: null` (engine: implicit 1% mode, distinct from key-absent = off) is stored in editor state as the sentinel `window.PGEYaml.DEVIATION_PROBABILITY_IMPLICIT` and serialized back to `deviation_probability: null`.

That key was called `dephase` until PGE v7.0.0 (PGE #204), renamed with **no back-compat alias** — and the engine's `StreamConfig.from_yaml` picks its fields *by name*, so the old spelling isn't rejected, it's **dropped**: a stream would render with `deviation_probability=False` while the YAML still read `dephase: 50`, silently. `dephase` therefore stays in `KNOWN_STREAM_KEYS` and is **healed at parse** (same treatment as `loop_duration`: a key the engine never knew, written by older editor builds), re-emitted under the current name. With both keys in the file the live one wins and the dead one is dropped — what the render already does. The healing is recorded in `deviationProbabilityLegacy`, a provenance flag that the Inspector surfaces (the file will be rewritten on save) and that is in `FP_IGNORE`, so reopening a pre-v7 project doesn't mark every stem stale over a key spelling. #124
