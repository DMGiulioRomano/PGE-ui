...
`grain.duration_unit` (`seconds | samples | milliseconds`, PGE #158 then #171) is the same shape of problem one level down: the engine's `grain_duration` bounds are in **seconds**, the YAML values are in the declared unit. `grainUnitFactor` / `grainUnitBounds` / `grainDefaultDuration` / `grainUnitSuffix` in `envelope-utils.js` are the single source — bounds, the `0.05` s default and the row suffix expressed in the unit in force (in ms the cap is `10000`, not `10`); they drive the EnvelopeEditor `hardMin/hardMax` + vis window and the seed of the scalar↔env toggle. Changing the unit goes through `convertGrainDurationUnit`, which **converts** `duration`/`duration_range` — scalars and envelopes, every form `Envelope._scale_raw_values_y` scales — instead of letting the old number be reinterpreted in the new scale, then re-clamps the scalars (envelope points need no clamp: bounds scale by the same factor). An unknown unit converts nothing and gets no suffix. The key is deleted only for `seconds` — absence *is* seconds. Two asymmetries are deliberate: the conversion applies only to what the **engine** scales (`isEngineEnvelopeLike` mirrors `Envelope.is_envelope_like` — a list of only dict breakpoints or only 3-tuples is not envelope-like there, so the engine reads it in seconds whatever the declared unit, and the UI must not rewrite those numbers); and changing the unit **does** mark the stem stale even though the rendered audio is identical, because `fingerprintStream` sees `0.05` become `50` — the safe direction (one render too many, never one too few), and normalizing the hash to seconds would cost more than it's worth.

Because of the first asymmetry the unit is **per curve, not per stream**: the EnvelopeEditor picks each duration curve's suffix and `hardMin/hardMax/vis` through `isEngineEnvelopeLike`, falling back to seconds for a curve the engine won't scale, and the Inspector prints a line next to the selector saying why. Labelling such a curve `ms` would put two domains on one axis and the first drag would write in the wrong one — `50` read off the axis, 50 **seconds** for the engine. One more engine-side quirk the mirror does *not* copy: inside a compact block `_scale_raw_values_y` drops the per-point interp (`[[p[0], p[1]*f] …]`); the UI keeps it, because copying it would delete an authored field, and the engine loses it at every render under a non-seconds unit anyway.
