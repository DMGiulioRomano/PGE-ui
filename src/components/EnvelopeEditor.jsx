...
function listEnvelopes(stream, sampleDur) {
  ...
  // (window.PGEDeviationProbability), so the typed {type, points} form (cubic
  // global interp) registers as the global env — not silently dropped.
  // isEnvValue is true for [[t,v],…] and {type, points}; the per-param branch
  // only runs when the value is NOT itself an env (i.e. a real per-param dict).
  const PGEDevProb = window.PGEDeviationProbability;
  const _dp = stream.deviationProbability;
  if (PGEDevProb.isEnvValue(_dp)) {
    list.push({ key: "deviationProbability", label: "probability", group: "Deviation",
      path: ["deviationProbability"], unit: "%",
      visMin: 0, visMax: 100, hardMin: 0, hardMax: 100 });
  } else if (PGEDevProb.mode(_dp) === "perParam") {
    const DEVIATION_PROBABILITY_PARAM_KEYS = ["volume","pan","duration","pitch","pointer","reverse","read_direction","envelope"];
    for (const pk of DEVIATION_PROBABILITY_PARAM_KEYS) {
      if (PGEDevProb.isEnvValue(_dp[pk])) {
        list.push({ key: "deviationProbability_" + pk, label: pk, group: "Deviation",
